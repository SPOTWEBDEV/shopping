...
if (isset($_GET['del_id'])) {
     ...
     $uppdate_clients = mysqli_query($connection, "DELETE FROM product_table WHERE id='$del_id'");
...
<head>
     ...
     <meta charset="utf-8">
     <title><?php echo 'Product-Page' .  '-' . $sitename ?> </title>
               ...
               <div class="group-data-[sidebar-size=lg]:ltr:md:ml-vertical-menu group-data-[sidebar-size=lg]:rtl:md:mr-vertical-menu group-data-[sidebar-size=md]:ltr:ml-vertical-menu-md group-data-[sidebar-size=md]:rtl:mr-vertical-menu-md group-data-[sidebar-size=sm]:ltr:ml-vertical-menu-sm group-data-[sidebar-size=sm]:rtl:mr-vertical-menu-sm pt-[calc(theme('spacing.header')_*_1)] pb-[calc(theme('spacing.header')_*_0.8)] px-4 group-data-[navbar=bordered]:pt-[calc(theme('spacing.header')_*_1.3)] group-data-[navbar=hidden]:pt-0 group-data-[layout=horizontal]:mx-auto group-data-[layout=horizontal]:max-w-screen-2xl group-data-[layout=horizontal]:px-0 group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:ltr:md:ml-auto group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:rtl:md:mr-auto group-data-[layout=horizontal]:md:pt-[calc(theme('spacing.header')_*_1.6)] group-data-[layout=horizontal]:px-3 group-data-[layout=horizontal]:group-data-[navbar=hidden]:pt-[calc(theme('spacing.header')_*_0.9)]">
                    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
                         ...
                         <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                              <div class="grow">
                                   <h5 class="text-16 capitalize"><?php echo $pageName ?></h5>
                              </div>
                              <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                                   <li class="relative before:content-['\ea54'] before:font-remix ltr:before:-right-1 rtl:before:-left-1  before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:text-slate-400 dark:text-zink-200">
                                        <a href="#!" class="text-slate-400 dark:text-zink-200">Product </a>
                                   </li>
                                   <li class="text-slate-700 dark:text-zink-100 capitalize">
                                        <?php echo $pageName ?>
                                   </li>
                              </ul>
                         </div>
                         ...
                         <div class="card">
                              <div class="card-body">
                                   <h6 class="mb-4 text-15">All Product Table</h6>
                                   <table id="basic_tables" class="display stripe group" style="width:100%">
                                        ...
                                        <tbody>
                                             ...
                                             if(isset($_GET['status'])){
                                                  if($_GET['status'] == 'pending' || $_GET['status'] == 'declined' || $_GET['status'] == 'approved' || $_GET['status'] == 'Approved'  ){
                                                       $get = $_GET['status'];
                                                       $select = mysqli_query($connection, "SELECT product_table.*, client.email ,client.username FROM product_table,client WHERE client.id=product_table.user_id AND product_table.status = '$get'");
                                                  }else{
                                                       $select = mysqli_query($connection, "SELECT product_table.*, client.email ,client.username FROM product_table,client WHERE client.id=product_table.user_id");
                                                  }
                                             }else{
                                                  $select = mysqli_query($connection, "SELECT product_table.*, client.email ,client.username FROM product_table,client WHERE client.id=product_table.user_id");
                                             }
                                             ...
                                             while ($row = mysqli_fetch_assoc($select)) {
                                                  ...
                                                  if($row['status'] == 'decline'){
                                                       $statuColor = 'bg-yellow-500';
                                                  }else if($row['status'] == 'approved'){
                                                       $statuColor = 'bg-green-500';
                                                  }else{
                                                            $statuColor = 'bg-red-500';
                                                  }
                                             ...
                                             ?>
                                                  <tr>
                                                       ...
                                                       <td> <img id="pics" src=".././uploads/products/<?php echo $row['product_front_photo']?>" alt="no"></td>
                                                       <td><?php echo $row['product_name'] ?></td>
                                                       <!-- <td></td> -->
                                                      
                                                       <td>
                                                            
                                                            <button class="<?php echo $statuColor ?> text-white capitalize " style="height: fit-content; width: fit-content; border:2px solid black; padding:8px 8px; border-radius:5px;"><?php echo $row['status'] ?></button>
                                                       ...
                                                       <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                                            <div class="relative dropdown">
                                                                 ...
                                                                 <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="orderAction1">
                                                                      ...
                                                                      <li>
                                                                           <a data-modal-target="addOrderModal" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="./more.php?item=<?php echo $row['id'] ?>"><i data-lucide="file-edit" class="inline-block size-3 ltr:mr-1 rtl:ml-1"></i> <span class="align-middle">View-more</span></a>
                                                                      </li>
                                                                      <li>
