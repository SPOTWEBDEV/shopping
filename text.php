...
<body>
    ...
    function getVariantIdFromURL() {
        ...
        return params.get('id'); // assumes the URL has ?id=12345
    }
    ...
    async function loadAndRenderProduct() {
        ...
        const matchedProduct = products.find(product => product.variant_id == variantIdFromUrl);

        if (matchedProduct) {
            // You can now render this matched product
            document.querySelector('.product_info_wrapper').innerHTML = renderProductCard(matchedProduct);
        } else {
    ...
    function renderProductCard(product) {
        ...
        //     discountText =
        //         `<div class="absolute top-[15px] left-[15px] p-2 rounded-[3px] bg-[#ffc107] text-[15px] text-white leading-[18px] z-10">${discount_percentage}% OFF</div>`;
        // }
    ...
    <script src="<?php echo $domain ?>assets/js/add_to_cart.js"></script>
    <script>
